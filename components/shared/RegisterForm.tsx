...
const RegisterForm = () => {
  ...
  return (
    <form
      ...
      <label
        htmlFor="email"
        className="w-full text-start font-medium text-lg mb-0.5 mt-2.5">
        Email:
      </label>
      <Input
        id="email"
      ...
      {message ? (
        message.includes("Error") ? (
          ...
          <p className="bg-green-200 rounded-lg text-base font-medium text-green-600 px-2.5 py-1.5 flex items-center gap-1.5 mt-2.5 self-start">
            <Check className="mt-0.5 w-5 h-5" />
            {message}
          </p>
        )
      ) : null}
